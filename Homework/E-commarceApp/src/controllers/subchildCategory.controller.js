...
const createSubChildCate = async (req, res) => {
  try {
    ...
    const subChildCategory = await subChildCateService.createSubChildCate(
      reqBody
    );
    if (!subChildCategory) {
...
const getSubChildCatelist = async (req, res) => {
  try {
    const getsubChildCategory = await subChildCateService.getSubChildCatelist(
      req,
      res
    );
    res.status(200).json({
...
const deleteSubChildcate = async (req, res) => {
  try {
    ...
    const subchildcateExi = await subChildCateService.getSubChildCateById(
      subchildcateId
    );
    if (!subchildcateExi) {
...
const updateSubChildCate = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "subchild category details update successfully!",
      });
  } catch (error) {
    res.status(400).json({
      success: false,
